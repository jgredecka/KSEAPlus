def userInput(df, min_sub, ks_db, graphics):
    ...
    # A mapping of phosphosite substrate keys to their (often multiple) log2(FC) values is achieved here.
    dic={}
    for entry in data:
        site=entry[0]
        fc=entry[1]
    ...
    # All log2(FC) values across all kinases are appended to a new list.
    # The mean log2(FC) and the standard deviation of all phosphosites are calculated next.
    # These values will be used to obtain the final z-score for each identified kinase later on.
    all_log2=[]    
    for key in dic:
    ...
    # Each phosphosite substrate in site_fc array is scanned against the EDGES K-S db. 
    ...
    # A dictionary containing unique kinases as keys and substrate log2(FC) as values is created.
    ...
    zscore_df=pd.DataFrame(zscore_info, columns=["Kinase", "Site", "mnlog2(FC)", "zSc", "pVal"])
